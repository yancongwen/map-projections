...
const texture = new TextureLoader().load('./static/images/world.jpg')
...
earthGeometry.vertices.map((v) => {
    ...
    state.cylinder.push(
        new Vector3(
            radius * Math.cos(lon),
            radius * Math.log(Math.tan(Math.PI / 4 + lat / 2)),
            radius * Math.sin(lon)
        )
    )
...
const steps = [...document.querySelectorAll('.steps .step-state')]
steps.map((dom, i) => {
